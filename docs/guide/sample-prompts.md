...
```
I built a CLI habit tracker with a Squad last month in another repo. I exported the squad with `squad export` and I have the squad-export.json file ready.

Import it into this repo with `squad import squad-export.json`.
...
```
Before we start, run `squad` to make sure we're on the latest version.
...
```
Import the squad from my previous project: squad import squad-export.json
```
